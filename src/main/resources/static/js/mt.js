   ...
   try
	...
	if(g_viewStatu[g_clicked].vbPlaying)
	{
		if(req=="start")		
    	{
			...
			xmlHttp.open("POST","http://192.168.1.101",true);
...
switch(iLanguage)
{
    ...
    case 101:
    {
        document.write("<script src='/js/SimpChinese.js'><\/script>");
        break;
    }
    default:
    {        
...
function Logout() {
   ...
   var gHashCookie = null;//new Hash.Cookie('NetSuveillanceWebCookie',{duration: 30});
var settings = {
	autoPrompt:''
	}
